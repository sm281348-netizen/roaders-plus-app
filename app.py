...
def _get_all_purchase_clean():
    ...
    _date_col = next((c for c in df_raw.columns if any(k in c for k in ['日期', 'Date', '採購日', '輸出', '日採'])), None)
    ...
    if not (_date_col and _item_col and _qty_col):
        return pd.DataFrame()
    df = df_raw.copy()
    # 日期解析：先嘗試民國年格式（如 0115/04/08），再 fallback 到標準西元格式
    import re as _re_d
    def _parse_purchase_date(val):
        s = str(val).strip()
        # 民國年：3位數（含前置0），如 0115/04/08 或 115/04/08
        _m = _re_d.match(r'^0?(\d{3})[/-](\d{1,2})[/-](\d{1,2})$', s)
        if _m:
            year_int = int(_m.group(1))
            if 90 <= year_int <= 200:
                try:
                    return pd.Timestamp(f'{year_int + 1911}-{int(_m.group(2)):02d}-{int(_m.group(3)):02d}')
                except Exception:
                    pass
        # 標準西元格式 fallback
        try:
            ts = pd.to_datetime(s, errors='coerce')
            if pd.notna(ts) and ts.year >= 1990:
                return ts
        except Exception:
            pass
        return pd.NaT
    df['_date_parsed'] = df[_date_col].apply(_parse_purchase_date)
                ...
                try:
                    if not df_month.empty and not df_prev_month.empty:
                        ...
                        if not m_df.empty and not pm_df.empty:
                            ...
                            def _get_guest_sum(fb_df):
                                if fb_df.empty: return 0
                                if 'bf_act' in fb_df.columns and 'af_act' in fb_df.columns:
                                    bf = pd.to_numeric(fb_df['bf_act'], errors='coerce').fillna(0)
                                    af = pd.to_numeric(fb_df['af_act'], errors='coerce').fillna(0)
                                    g = (bf + (af * 1.5)).sum()
                                    if g > 0: return g
                                g_col = 'effective_peak_guests' if 'effective_peak_guests' in fb_df.columns else 'peak_guests'
                                if g_col in fb_df.columns:
                                    return pd.to_numeric(fb_df[g_col], errors='coerce').fillna(0).sum()
                                return 0
                                
                            curr_guests_total = _get_guest_sum(curr_fb)
                            prev_guests_total = _get_guest_sum(prev_fb)
                            
                            def check_sync(r):
                                if curr_guests_total == 0 or prev_guests_total == 0:
                                    return "無客數資料"
                                
                                curr_qty = r['本月總量']
                                prev_qty = r['上月總量']
                                
                                if prev_qty <= 0: return "上月無叫貨"
                                if curr_qty <= 0: return "本月無叫貨"
                                
                                curr_upg = curr_qty / curr_guests_total
                                prev_upg = prev_qty / prev_guests_total
                                
                                upg_growth = (curr_upg - prev_upg) / prev_upg
                                
                                if upg_growth > 0.15:
                                    return f"🚨 耗用暴增 {upg_growth*100:.0f}%(客少卻多買)" if curr_guests_total < prev_guests_total and curr_qty > prev_qty else f"⚠️ 耗用增加 {upg_growth*100:.0f}%"
                                elif upg_growth < -0.15:
                                    return f"📉 耗用減少 {abs(upg_growth)*100:.0f}%"
                                else:
                                    return "✅ 完美匹配客數"
...
if selected_page == "💰 採購分析":
    ...
    if 'df_purchase' in locals() and df_purchase is not None and not df_purchase.empty:
        ...
        st.caption(
            "根據過去 90 天歷史採購資料，計算單品每客耗用率 (UPG)，"
            "並結合最近一次叫貨時的庫存基準，推算理論剩餘量與安全叫貨建議。"
        )
        ...
        # 從 get_purchase_data_cached 取得所有採購資料（含 90 天）
        df_purchase_all = get_purchase_data_cached()
        df_purchase_all_clean = pd.DataFrame()

        if df_purchase_all is not None and not df_purchase_all.empty:
            df_purchase_all.columns = df_purchase_all.columns.astype(str).str.strip()
            _date_col_inv = next((c for c in df_purchase_all.columns if '日期' in c or 'Date' in c), None)
            _item_col_inv = next((c for c in df_purchase_all.columns if any(k in c for k in ['品名', '品項', '項目', 'Item'])), None)
            ...
            if _date_col_inv and _item_col_inv and _qty_col_inv:
                ...
                # 數量清理
                df_purchase_all_clean['_qty'] = pd.to_numeric(
                    df_purchase_all_clean[_qty_col_inv].astype(str).str.replace(',', ''), errors='coerce').fillna(0)
...
if current_hotel != "採購":
    if selected_page == "👥 人事概況":
        ...
        if df_emp.empty:
            ...
            # 獎金發放標準與加權公式說明
            with st.expander("📖 獎金發放標準、加權規則與計算公式說明（點擊展開/收合）", expanded=True):
                st.markdown("""
                ### 🎯 獎金分配核心機制（模型 B：純點數積分制）
                本系統採用**「純點數積分制」**，依據每位同仁的**職位職階**、**年資累積**以及**本期績效考核分數**進行公平、透明的加權分配，並嚴格控管總發放金額不超過預算上限。
                ...
                * **名單鎖定**：點擊「儲存本次發放名單與考核分數」後，系統即固定保存此名單與分數，重整頁面時自動載入。
                ...
                with col_r2:
                    st.markdown("""
                    **⏳ 年資點數 (B)**
                    * **計算方式**：年資（年數）精確採計至小數點後兩位。
                    * **上限規範**：年資點數最高上限為 **10 點**（年資滿 10 年者最高計 10 點）。
                    * **試算範例**：滿 1.5 年 = 1.5 點；滿 4 年 2 個月 ≈ 4.17 點。
                    """)
                ...
                st.latex(r"\text{個人總點數} = (\text{職位點數} + \text{年資點數}) \times \left( \frac{\text{績效評核分數}}{100} \right)")
                st.latex(r"\text{每點價值 (NT\$)} = \frac{\text{本期總獎金池額度}}{\sum \text{所有合格同仁的總點數}}")
                st.latex(r"\text{建議分配獎金} = \left\lfloor \frac{\text{個人總點數} \times \text{每點價值}}{100} \right\rfloor \times 100 \quad \text{(無條件捨去至百位數)}")
                
                st.info("💡 **無條件捨去規則**：為確保總發放金額絕對不超過設定的總獎金池上限，每位同仁的建議金額均自動無條件捨去至百位數（如 NT$ 12,385 → NT$ 12,300），剩餘零頭保留於公司預算中。")
...
def render_free_services_optimization_tab():
    ...
    default_price_map = {
        ...
        "台啤-芒果": 28.33, "台啤-鳳梨": 28.33, "樹頂100%蔓越莓汁(玻璃瓶)": 36.0, "金沙巧克力(心形八入組) - 12盒": 16.0,
        "明信片(小迷鹿)": 3.0, "明信片(府前)": 3.0, "明信片(東門)": 3.0,
        "明信片(谷墨)": 3.0, "明信片(路徒PLUS)": 3.0, "三合一牛奶": 7.5, "玉米鬚茶": 3.25, "205紙杯7oz(私版)": 0.81,
        "HJ-90咖啡蓋-黑色": 1.0, "餐車4F(扣除紙杯蓋)": 250.0, "洗衣粉(單包裝)": 4.2, "熊寶貝自然草本衣物清新噴霧": 79.0
    }
    default_monthly_qty = {
        "我的美麗日記面膜(玻尿酸)": 300, "蒸氣眼罩(一次性)": 350, "迷你迴力玩具車": 80, "樹頂100%蘋果汁": 450,
        "台啤-芒果": 100, "台啤-鳳梨": 100, "樹頂100%蔓越莓汁(玻璃瓶)": 120, "金沙巧克力(心形八入組) - 12盒": 180,
        "明信片(小迷鹿)": 100, "明信片(府前)": 100, "明信片(東門)": 100,
    ...
    try:
        ...
        with col_b_guide:
            st.markdown("#### 🧭 4 象限決策推導指南")
            st.info(
                f"**📐 分割中線基準**：\n"
                f"- **X 軸 (感知中線)**：固定 `3.0 分`\n"
                f"- **Y 軸 (花費中線)**：動態中位數 `NT$ {int(mid_y):,} / 月`"
            )

            st.markdown(
                "**↖️ 第一象限（左上：高花費 ｜ 低感知）**\n"
                "🚨 **優先處置區 (成本痛點)**：花大錢但住客體感低。\n"
                "➔ **建議處置**：`直接刪除` / `改為被動索取`\n\n"
                "**↗️ 第二象限（右上：高花費 ｜ 高感知）**\n"
                "⚖️ **核心控制區 (質感維持)**：住客有感但花費大。\n"
                "➔ **建議處置**：`平價替代` / `精準控量`\n\n"
                "**↙️ 第三象限（左下：低花費 ｜ 低感知）**\n"
                "🧹 **微幅簡化區**：預算影響小、住客無感。\n"
                "➔ **建議處置**：`機動精簡` 或 `維持現狀`\n\n"
